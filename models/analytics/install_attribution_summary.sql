with install_summary as (
    select
        ...
        count(distinct case when campaign_id = -1 then customer_id end) as organic_installs,
        count(distinct case when campaign_id != -1 then customer_id end) as paid_installs
...
order by platform, install_count desc