WITH campaign_metrics AS (
    ...
    WHERE is_high_value_campaign = 'TRUE'
    AND total_num_installs > 0
)
SELECT 
    cm.campaign_id,
    cm.campaign_name,
    ...
    CASE 
        WHEN cm.install_duration_ratio < 1 THEN 'Fast Converting'
        WHEN cm.install_duration_ratio < 2 THEN 'Average'
        ELSE 'Slow Converting'
    END AS conversion_speed
FROM campaign_metrics cm
WHERE cm.avg_customer_acquisition_cost < (
    SELECT AVG(avg_customer_acquisition_cost)
    FROM dim_marketing_campaigns
)
