...
create or replace view MOMS_FLOWER_SHOP.ANALYTICS.DIM_MARKETING_CAMPAIGNS as (SELECT
    ...
    m.total_campaign_spent / NULLIF(i.total_num_installs, 0) AS avg_customer_acquisition_cost,
    m.campaign_duration / NULLIF(i.total_num_installs, 0) AS install_duration_ratio,
    COALESCE(m.total_campaign_spent > 1000, FALSE) AS is_high_value_campaign
